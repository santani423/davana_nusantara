                    ...
                    <div class="body">
                        <form id="mediaForm" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="mt-3">
                                    <img id="thumbnail_preview"
                                        src="{{ isset($media) ? asset($media->thumbnail_img) : '#' }}" alt="Image Preview"
                                        style="{{ isset($media) ? '' : 'display: none;' }} max-width: 100%; height: auto;">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="resume" id="resume" class="form-control" rows="4" required>{{ $media->resume ?? '' }}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="content">Content</label>
                                <textarea name="content" id="content" class="summernote" rows="6" required>{!! $media->content ?? '' !!}</textarea>
                            </div>
...
@section('script')
    ...
    <script>
        ...
        function simpanData() {
            ...
            const isEdit = {{ isset($media) ? 'true' : 'false' }};

            $.ajax({
                url: "{{ url('/api/ruang-media/update/' . $code) }}",
                method: "post",
                headers: {
                ...
                success: function(res) {
                    ...
                    Swal.fire({
                        ...
                        window.location.href = "{{ url('/cms/ruang_media') }}";
                    });
                },
                error: function(err) {
                    $('#loadingModal').modal('hide');
